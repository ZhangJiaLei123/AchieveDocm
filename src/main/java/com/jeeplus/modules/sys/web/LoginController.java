...
//		else if (null != role &&"student".equals(UserUtils.getUser().getRole().getEnname())){
//			message = "当前用户是学员，请从学员端登陆.";
//		}
...
//		System.out.println("==========================b");
		...
		User us =  userDao.get(UserUtils.getUser().getId());
		try {
			//查询用户等级
			User queryUser = new User();
			queryUser.setUserId(UserUtils.getUser().getId());
			User userGarde = userDao.getUserScore(queryUser);
			if(userGarde==null) {
				queryUser.setGradeScore(0);
			}
			queryUser.setGradeScore(userGarde.getGradeScore());
			User gradeName = userDao.getUserGradeName(queryUser);
			if(gradeName==null||gradeName.getGradeName()==null||"".equals(gradeName.getGradeName())) {
				UserUtils.getUser().setGradeName("初级");
			}else {
				UserUtils.getUser().setGradeName(gradeName.getGradeName());
			}
		}catch(Exception e) {
			
		}
		
		if(null != UserUtils.getUser().getRole() &&("nx_teacher".equals(UserUtils.getUser().getRole().getEnname())||"teacher".equals(UserUtils.getUser().getRole().getEnname()))){
			//如果是内训师判断是否审核通过
			if(null != UserUtils.getUser().getRole() && ("nx_teacher".equals(UserUtils.getUser().getRole().getEnname()))){
				if("0".equals(us.getShStatus())){
					return "redirect:/a/teacherindex";
				}else{
					return "modules/sys/studentLogin";
				}
			}
			return "redirect:/a/teacherindex";
			
		}else if(null != UserUtils.getUser().getRole() && "student".equals(UserUtils.getUser().getRole().getEnname())){
			return "modules/sys/studentLogin";
		}
		else {
			return "modules/sys/sysIndex";
