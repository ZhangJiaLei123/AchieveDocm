...
public class LuceneCreateIndexByDirectory {
	
	public static void main(String[] args) {
		try {
			createIndex("E:\\测试文件目录\\source", "E:\\测试文件目录\\index");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	 ...
	 * @throws IOException
	 */
	public static void createIndex(String dataDirectory,String indexDirectory) throws IOException {
	    ...
	    create(dataDirectory, indexWriter);
	    // 关闭索引
	    indexWriter.close();
	}
        ...
        try {
            ...
            for (int i = 0; i < files.length; i++) {
            	if(files[i].isDirectory()) {
            		//如果是文件夹，则递归
            		create(files[i].getAbsolutePath(), indexWriter);
            		continue;
            	}
                // 文件是第几个
                System.out.println("这是第" + i + "个文件----------------");
                // 文件的完整路径
                String allPath = files[i].toString();
