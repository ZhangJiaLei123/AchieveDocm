...
<head>
	...
	<script type="text/javascript">
		...
		function changeUserSel(){
			var type = window.frameElement && window.frameElement.id || '';
			window.parent.changeUserSelFalse(type);
		}
	</script>
...
<body>
	<form:form id="searchForm" modelAttribute="office" action="${ctx}/sys/office/selOfficeList/" method="post">
		...
		<table style="width: 100%">
			<tr>
				<td width="60px"><input type="checkbox" value="1" id="checkAllOffice" onclick="checkAllOfficeFun(this)"  style="width: 15px; height: 25px;border:none"  name="checkAllOffice"/><label style="cursor:pointer;" for="checkAll">全选</label></td>
...
<!-- 			<th><input type="checkbox" id="checkAll" style="width: 15px; height: 25px;border:none" onclick="checkBoxAll(this)"></th> -->
		...
		<c:forEach items="${page.list}" var="office">
			<tr>
				<td><input type="checkbox"  style="width: 15px; height: 25px;border:none"  name = "chk" value = "${office.id}" onclick="changeOffic(this)" id="${office.id}" ></td>
				<td>${office.name}</a></td>
				<td>${office.code}</td>
