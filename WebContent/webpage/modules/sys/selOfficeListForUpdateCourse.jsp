...
<body>
	<form:form id="searchForm" modelAttribute="office" action="${ctx}/sys/office/selOfficeListForUpdateCourse" method="post">
		...
		<table style="width: 100%">
			<tr>
				<td width="60px"><input type="checkbox" value="1" id="checkAllOffice" onclick="checkAllOfficeFun(this)"  name="checkAllOffice" <c:if test="${condition.orgCheckAll==1}">checked</c:if>/><label style="cursor:pointer;" for="checkAllOffice">全选</label></td>
			...
			<td width="5px"></td>
			<td><button  class="btn btn-primary btn-rounded btn-outline btn-sm " onclick="doSubmit()" ><i class="fa fa-search"></i>搜索</button></td>
			</tr>
		</table>
		</div>
	</form:form>
	...
	<table id="officeTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable" style="width:100%;">
		...
		<c:forEach items="${page.list}" var="office">
			<tr>
				<td width="20px"><input type="checkbox"  style="width: 15px; height: 25px;border:none"  name = "chk" value = "${office.id}" onclick="changeOffic(this)" id="${office.id}" <c:if test="${office.checked==1}">checked</c:if>></td>			<td>${office.name}</a></td>
				<td>${office.code}</td>
