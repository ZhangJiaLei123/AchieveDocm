...
<head>
	...
	<script type="text/javascript">
		...
		function changeIsCheckPostAll(obj,ids){
			var bl = obj.checked;
			if(bl){
				$("#isCheckPost",parent.document).val("1");
			}else{
				$("#isCheckPost",parent.document).val("");
				$("#postIds",parent.document).val(ids);
			}
		}
		...
		function changeCheckPostIds(obj){
			var bl = obj.checked;
			var postIdsObj =  $("#postIds",parent.document);
...
<div style="padding:5px;">
	<!--查询条件-->
	<form:form id="searchForm" modelAttribute="postInfo" action="${ctx}/sys/postInfo/showPostListByConditionCourse" method="post" class="form-inline">
		<!-- 以此来判断是必学，还是选学的赋值 -->
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		...
		<tbody>
		<c:forEach items="${page.list}" var="objPost">
			<tr>
				<td> <input type="checkbox" name="chk" onclick="uncheckAll(this);" value="${objPost.positLevelId}" 
				postName ="${objPost.name}" postLevelName="${objPost.postLevelName}" id="${objPost.positLevelId}" >
				</td>
				<td>
					${objPost.name}
				</td>
				<td>
					${objPost.postLevelName}
				</td>
				<td>
					${objPost.postinfoTypeName}
				</td>
			</tr>
		</c:forEach>
		</tbody>
